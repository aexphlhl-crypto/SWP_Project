...
public class RoomService {
    ...
    public List<SeatConfigDto> configureSeats(Long roomId, List<SeatConfigDto> seatConfigs) {
        ...
        if (!"UnderMaintenance".equals(room.getStatus()) && !"Inactive".equals(room.getStatus())) {
            throw AppException.badRequest("Sơ đồ ghế chỉ có thể cấu hình khi phòng chiếu đang bảo trì hoặc ngừng hoạt động.");
        }
        ...
        if (!seatsToDelete.isEmpty()) {
            try {
                ...
                throw AppException.badRequest("Không thể xóa các ghế đang được giữ hoặc đã có người đặt.");
            }
        }

        List<Seat> savedSeats = seatRepository.saveAll(seatsToSave);
        
        int newCapacity = (int) savedSeats.stream()
                .filter(s -> s.getSeatType() != SeatType.Hidden)
                .count();
