...
public class EmailService {
    ...
    public void sendManagerCredentials(String to, String fullName, String tempPassword) {
        ...
        String content = "<div style='font-family:Arial,sans-serif;max-width:600px;margin:0 auto;'>"
                ...
                + "<p style='margin:4px 0;'><strong>Mật khẩu tạm thời:</strong> <code style='background:#e0e0e0;padding:2px 6px;border-radius:4px;'>"
                + tempPassword + "</code></p>"
                + "</div>"
    ...
    public void sendBookingConfirmation(String to, com.cinebook.backend.modules.bookings.dto.MyBookingDto booking) {
        ...
        content.append(
                "<tr><td style='padding: 8px; border: 1px solid #ddd;'><strong>Mã đặt vé:</strong></td><td style='padding: 8px; border: 1px solid #ddd;'>")
                .append(booking.getId()).append("</td></tr>");
        content.append(
                "<tr><td style='padding: 8px; border: 1px solid #ddd;'><strong>Phim:</strong></td><td style='padding: 8px; border: 1px solid #ddd;'>")
                .append(booking.getMovieTitle()).append("</td></tr>");
        content.append(
                "<tr><td style='padding: 8px; border: 1px solid #ddd;'><strong>Rạp:</strong></td><td style='padding: 8px; border: 1px solid #ddd;'>")
                .append(booking.getCinemaName()).append("</td></tr>");
        content.append(
                "<tr><td style='padding: 8px; border: 1px solid #ddd;'><strong>Phòng chiếu:</strong></td><td style='padding: 8px; border: 1px solid #ddd;'>")
                .append(booking.getRoomName()).append("</td></tr>");
        content.append(
                "<tr><td style='padding: 8px; border: 1px solid #ddd;'><strong>Ngày chiếu:</strong></td><td style='padding: 8px; border: 1px solid #ddd;'>")
                .append(booking.getShowDate()).append("</td></tr>");
        content.append(
                "<tr><td style='padding: 8px; border: 1px solid #ddd;'><strong>Giờ chiếu:</strong></td><td style='padding: 8px; border: 1px solid #ddd;'>")
                .append(booking.getShowTime()).append("</td></tr>");
        content.append(
                "<tr><td style='padding: 8px; border: 1px solid #ddd;'><strong>Ghế:</strong></td><td style='padding: 8px; border: 1px solid #ddd;'>")
                .append(booking.getSeatNumber()).append("</td></tr>");
        content.append(
                "<tr><td style='padding: 8px; border: 1px solid #ddd;'><strong>Tổng tiền:</strong></td><td style='padding: 8px; border: 1px solid #ddd;'>")
                .append(String.format("%,d VND", booking.getTotalAmount())).append("</td></tr>");
        ...
        if (booking.getFnbItems() != null && !booking.getFnbItems().isEmpty()) {
            ...
            for (com.cinebook.backend.modules.bookings.dto.FnBItemDto item : booking.getFnbItems()) {
                content.append("<li>").append(item.getProductName()).append(" (x").append(item.getQuantity())
                        .append(")</li>");
            }
            content.append("</ul>");
        }
        ...
        content.append(
                "<p style='text-align: center; color: #666; font-size: 12px;'>Vui lòng xuất trình các mã QR dưới đây tại quầy kiểm soát vé hoặc quầy bắp nước.</p>");
        ...
        if (booking.getTickets() != null) {
            for (com.cinebook.backend.modules.bookings.dto.TicketDto ticket : booking.getTickets()) {
                String encodedQr = "";
                try {
                    encodedQr = java.net.URLEncoder.encode(ticket.getQrCodeValue(),
                ...
                content.append(
                        "<div style='display: inline-block; margin: 10px; padding: 15px; border: 1px solid #eee; border-radius: 8px;'>");
                content.append("<img src='").append(qrUrl).append("' alt='QR Code' />");
                content.append("<p style='margin: 10px 0 0 0; font-family: monospace; font-weight: bold;'>")
                        .append(ticket.getTicketCode()).append("</p>");
                String label = "FNB".equals(ticket.getSeatType()) ? "Bắp nước" : "Ghế " + ticket.getSeatLabel();
                content.append("<p style='margin: 5px 0 0 0; font-size: 14px; color: #555;'>").append(label)
                        .append("</p>");
                content.append("</div>");
            }
        }
        ...
        content.append(
                "<p style='text-align: center; font-size: 12px; color: #999;'>Đây là email tự động. Vui lòng không trả lời email này.</p>");
        content.append("</div>");
