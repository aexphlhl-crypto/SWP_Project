...
public class DashboardService {
    ...
    public List<ChartResponse> getRevenueChart() {
        ...
        List<Booking> bookings = bookingRepository.findByStatusInAndCreatedAtBetween(validStatuses, startOfYear,
                endOfYear);
    ...
    public byte[] exportRevenueToExcel() throws IOException {
        ...
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            for (Booking booking : bookings) {
                ...
                row.createCell(1)
                        .setCellValue(booking.getShowtime() != null && booking.getShowtime().getShowtimeId() != null
                                ? booking.getShowtime().getShowtimeId().toString()
                                : "");
    ...
    public List<MovieRankingResponse> getTopMoviesByRevenue() {
        ...
        return results.stream().map(row -> new MovieRankingResponse(
                ...
                (String) row[1],
                new BigDecimal(((Number) row[2]).toString()))).collect(Collectors.toList());
    }
    ...
    public List<RecentBookingResponse> getRecentBookings(int limit) {
        ...
        return page.getContent().stream().map(b -> {
            ...
            String movieName = (b.getShowtime() != null && b.getShowtime().getMovie() != null)
    ...
    public List<GenreChartResponse> getGenreChart() {
        ...
        List<String> colors = Arrays.asList("#E50914", "#F59E0B", "#3B82F6", "#10B981", "#8B5CF6", "#EC4899",
                "#14B8A6");
    ...
    public List<CinemaChartResponse> getCinemaChart() {
        ...
        for (Map.Entry<String, int[]> entry : cinemaStats.entrySet()) {
            result.add(new CinemaChartResponse(entry.getKey(), entry.getValue()[0],
                    new BigDecimal(entry.getValue()[1] / 1000000.0)));
        }
    ...
    public List<WeekdayChartResponse> getWeekdayChart() {
        ...
        // Group by LocalDate to find total tickets per specific date, then average by
        // day of week
