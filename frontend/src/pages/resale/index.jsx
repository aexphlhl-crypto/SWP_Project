                      ...
                      <div className="space-y-3.5">
                        ...
                        <div className="space-y-1.5 text-xs text-muted-foreground">
                          ...
                          {listing.includesFnb && listing.fnbItems && listing.fnbItems.length > 0 ? (
                            <div className="flex flex-col gap-1 text-primary text-xs font-medium">
                              <div className="flex items-center gap-2 font-bold mb-0.5">
                                <span>🍿</span>
                                <span>Đã kèm Bắp & Nước:</span>
                              </div>
                              {listing.fnbItems.map(fnb => (
                                <div key={fnb.productId} className="pl-6 text-muted-foreground font-normal">
                                  • {fnb.productName} <span className="text-xs ml-0.5 opacity-80">×{fnb.quantity}</span>
                                </div>
                              ))}
                            </div>
                          ) : listing.includesFnb ? (
        ...
        <div className="fixed inset-0 z-50 bg-black/40 dark:bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in duration-200">
            ...
            <div className="flex gap-2.5 p-3 rounded-xl bg-amber-500/5 border border-amber-500/10 text-[11px] text-muted-foreground/60 leading-relaxed">
              ...
              <p>
                <strong>Khuyến cáo bảo mật:</strong> Để đảm bảo an toàn tuyệt đối, chỉ thực hiện chuyển khoản sau khi đã xác nhận mã vé trực tiếp.
              </p>
            </div>
          </div>
        </div>
