        ...
        for (let j = i + 1; j < roomShowtimes.length; j++) {
          ...
          if (gap < 20) {
            ...
            conflictAlerts.push({
              ...
              top: alertTop,
              height: alertHeight,
              msg: "Trùng lịch chiếu — Vi phạm thời gian dọn dẹp tối thiểu 20 phút"
            });
          }
        }
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-foreground tracking-tight">Quản lý lịch chiếu</h1>
          <p className="text-xs text-muted-foreground mt-1 tracking-wider">Quản lý và theo dõi các suất chiếu trong hệ thống</p>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card className="bg-card border-border rounded-2xl shadow-xl p-6 flex items-center justify-between">
          <div className="space-y-1">
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Tỷ lệ lấp đầy ghế trung bình</p>
            <div className="text-2xl font-black text-foreground text-center">{seatFillRate}%</div>
          </div>
        </Card>

        {/* Phim xem nhiều nhất card */}
        <Card className="bg-card border-border rounded-2xl shadow-xl p-6 flex items-center justify-between">
          <div className="space-y-1 flex-1 min-w-0">
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Phim xem nhiều nhất</p>
            <div className="text-sm font-black text-foreground truncate leading-tight mt-1 text-center">
      ...
      <Card className="bg-card border-border rounded-2xl shadow-xl overflow-hidden">
        <Tabs defaultValue="calendar"> {/* Default to calendar (Lịch phòng) as requested! */}
          <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border bg-card/80">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  className="w-44 pl-10 bg-muted border-border rounded-xl text-sm h-10 focus:border-primary/60"
                />
              </div>

              {/* Movie Select */}
              <Select value={selectedMovie} onValueChange={setSelectedMovie}>
                <SelectTrigger className="w-52 bg-muted border-border rounded-xl text-xs h-10">
            ...
            {/* Tab Swappers */}
            <TabsList className="bg-muted border border-border rounded-xl h-10 p-1 shrink-0">
              ...
              <Table>
                <TableHeader className="bg-muted/50">
                  <TableRow className="border-border hover:bg-transparent">
                    <TableHead className="text-muted-foreground font-bold uppercase text-[10px] tracking-wider pl-6">Phim</TableHead>
                    <TableHead className="text-muted-foreground font-bold uppercase text-[10px] tracking-wider">Rạp</TableHead>
                    <TableHead className="text-muted-foreground font-bold uppercase text-[10px] tracking-wider">Phòng</TableHead>
                    <TableHead className="text-muted-foreground font-bold uppercase text-[10px] tracking-wider">Giờ chiếu</TableHead>
                    <TableHead className="text-muted-foreground font-bold uppercase text-[10px] tracking-wider">Ghế trống</TableHead>
                    <TableHead className="text-muted-foreground font-bold uppercase text-[10px] tracking-wider">Trạng thái</TableHead>
                    <TableHead className="text-muted-foreground font-bold uppercase text-[10px] tracking-wider text-right pr-6">Tác vụ</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filtered.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={7} className="text-center py-12 text-muted-foreground text-sm font-semibold">
                    ...
                    return (
                      <TableRow key={s.showtimeId} className="border-border hover:bg-muted/50 transition-colors">
                        <TableCell className="font-extrabold text-foreground pl-6 max-w-[220px] truncate">{s.movieTitle}</TableCell>
                        <TableCell className="text-xs text-muted-foreground font-medium">{s.cinemaName}</TableCell>
                        ...
                        <TableCell className="text-xs font-semibold text-muted-foreground">{s.availableSeats}/{s.totalSeats} ghế</TableCell>
                        <TableCell>
                          <Badge className={cn(
                        ...
                        <TableCell className="text-right pr-6">
                          <div className="flex items-center justify-end gap-1">
                            <Button
                              variant="ghost"
                              size="sm"
                              className="gap-1.5 hover:bg-muted text-muted-foreground hover:text-foreground h-8 px-2 rounded-lg text-xs"
                            ...
                            {occupancy === 0 && (
                              <>
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  className="gap-1.5 hover:bg-muted text-blue-400 hover:text-blue-300 h-8 px-2 rounded-lg text-xs"
              ...
              <div className="min-w-[900px] space-y-6">
                {selectedCinema === 'all' ? (
                  <div className="text-center py-20 bg-card border border-border rounded-2xl">
                    ...
                    <h3 className="text-base font-bold text-foreground uppercase tracking-wider">Chọn Rạp để xem lịch</h3>
                    <p className="text-xs text-muted-foreground max-w-sm mx-auto mt-1">Vui lòng chọn 1 rạp cụ thể từ bộ lọc phía trên để xem Sơ đồ lịch chiếu trực quan.</p>
                  </div>
                ) : timelineRooms.length === 0 ? (
                  <div className="text-center py-20 bg-card border border-border rounded-2xl">
                    <Monitor className="w-12 h-12 text-zinc-700 mx-auto mb-3" />
                    <h3 className="text-base font-bold text-foreground">Không tìm thấy phòng chiếu</h3>
                    <p className="text-xs text-muted-foreground mt-1">Rạp được chọn hiện tại chưa được định cấu hình phòng chiếu nào.</p>
                  </div>
                ) : (
                  <div>
                    {/* Top Branch Header in Timeline */}
                    <div className="flex items-center justify-between mb-4 px-1">
                      <div className="flex items-center gap-2 bg-card border border-border rounded-xl px-4 py-2 text-foreground">
                    ...
                    <div className="grid grid-cols-9 gap-4 items-center mb-4 px-4 select-none">
                      ...
                      >
                        {timelineRooms.map((room, roomIdx) => {
                          ...
                          return (
                            <div key={room.roomId} className="bg-muted border border-border rounded-xl px-3 py-2 text-center">
                              <p className={cn("text-xs font-black", isVipRoom ? "text-primary" : "text-foreground")}>{room.name}</p>
                              <p className="text-[10px] text-muted-foreground font-bold mt-0.5">Sức chứa: {room.capacity} ghế</p>
                            </div>
                          );
                        })}
                      </div>
                    ...
                    <div className="grid grid-cols-9 gap-4 items-start relative bg-background rounded-2xl border border-border p-4">

                      {/* Left vertical time ruler column */}
                      <div className="col-span-1 h-[840px] relative text-[11px] font-black text-muted-foreground pr-4 select-none border-r border-border">
                      ...
                      <div className="col-span-8 grid grid-cols-4 gap-4 h-[840px] relative"
                        ...
                        {Array.from({ length: 8 }).map((_, i) => {
                          ...
                          return (
                            <div
                              key={i}
                              className="absolute left-0 right-0 h-[1px] border-t border-dashed border-border pointer-events-none"
                              style={{ top: `${topPos}px` }}
                            />
                          );
                        })}
                        ...
                        {timelineRooms.map((room, roomIdx) => {
                          ...
                          return (
                            <div key={room.roomId} className="relative h-full border-r border-border last:border-r-0 flex flex-col pt-1">
                              ...
                              <div className="flex-1 relative w-full h-[840px]">
                                  ...
                                  return (
                                    <div
                                      ...
                                      className={cn(
                                        ...
                                        hasConflict
                                          ? "border-red-500 bg-red-500/10 dark:bg-red-950/20 text-red-500 dark:text-red-400 hover:bg-red-500/20 dark:hover:bg-red-950/30"
                                          : isVipRoom
                                            ? "border-primary/40 bg-primary/5 hover:bg-primary/10 text-primary"
                                            : "border-blue-500/40 bg-blue-500/10 hover:bg-blue-500/20 text-blue-500"
                                      )}
                                      style={{ top: `${topPos}px`, height: `${cardHeight}px` }}
                                    >
                                      {/* Left side: Widescreen movie poster thumbnail */}
                                      <div className="w-8 h-10 rounded overflow-hidden shrink-0 border border-border">
                                      ...
                                      <div className="flex-1 min-w-0 h-full flex flex-col justify-between">
                                        <div className="space-y-0.5">
                                          <div className="flex items-center justify-between">
                                            <span className="text-[8px] font-black uppercase bg-muted border border-border px-1 py-0.5 rounded leading-none text-muted-foreground">
                                              {s.timeString} - {s.endTimeString}
                                            </span>
                                          </div>
                                          <h4 className="font-extrabold text-foreground text-[11px] leading-tight truncate group-hover:text-primary transition-colors mt-1">
                                            {s.movieTitle}
                                          </h4>
                                        </div>
                                        ...
                                        <div className="space-y-1">
                                          <div className="flex items-center justify-between text-[8px] font-bold text-muted-foreground leading-none">
                                            <span>{s.totalSeats - s.availableSeats}/{s.totalSeats} Ghế</span>
                                          </div>
                                ...
                                {roomAlerts.map((alert, idx) => (
                                  <div
                                    key={idx}
                                    className="absolute left-0.5 right-0.5 bg-red-500/15 dark:bg-red-950/85 border border-red-500/50 rounded-xl p-2 z-20 flex items-start gap-1.5 shadow-2xl animate-pulse cursor-default"
                                  ...
                                  >
                                    ...
                                    <div className="min-w-0">
                                      ...
                                      <p className="text-[9px] text-muted-foreground leading-tight mt-0.5 font-medium">{alert.msg}</p>
                                    </div>
                                  </div>
                                ))}
                    ...
                    <div className="flex items-center justify-center flex-wrap gap-4 text-xs text-muted-foreground py-4 border-t border-border mt-4">
                      <div className="flex items-center gap-1.5"><div className="w-3.5 h-3.5 rounded-md border border-[#2563eb]/40 bg-blue-500/10" /> Standard Room</div>
                      ...
                      <div className="flex items-center gap-1.5"><div className="w-3.5 h-3.5 rounded-md border border-red-500 bg-red-500/10 dark:bg-red-950/20" /> Trùng lịch</div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-[425px] bg-card border border-border text-foreground rounded-2xl shadow-2xl">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Phim chiếu *</Label>
              <Select value={formData.movieId} onValueChange={v => setFormData({ ...formData, movieId: v })}>
                <SelectTrigger className="w-full bg-muted border-border rounded-xl h-11 text-sm focus:border-primary/60">
                  <SelectValue placeholder="Chọn phim" />
                </SelectTrigger>
                <SelectContent className="bg-card border-border">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Rạp chiếu *</Label>
              ...
              <div className="space-y-1.5">
                <Label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Phòng chiếu *</Label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Ngày chiếu *</Label>
                <Input
                  type="date"
                  value={formData.date}
                  onChange={e => setFormData({ ...formData, date: e.target.value })}
                  className="bg-muted border-border rounded-xl h-11 text-sm focus:border-primary/60"
                />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Giờ bắt đầu *</Label>
                <Input
                  type="time"
          ...
          <DialogFooter className="border-t border-border pt-3 mt-2 flex justify-end gap-2">
      ...
      <Dialog open={isSeatsDialogOpen} onOpenChange={setIsSeatsDialogOpen}>
        <DialogContent className="sm:max-w-[600px] max-h-[85vh] overflow-y-auto bg-card border border-border text-foreground rounded-2xl shadow-2xl">
          <DialogHeader className="border-b border-border pb-3">
            <DialogTitle className="text-lg font-black tracking-tight text-foreground">
              Sơ đồ ghế - Suất chiếu {selectedShowtimeForSeats?.timeString}
            </DialogTitle>
          </DialogHeader>
          <div className="py-2 text-xs space-y-1 text-muted-foreground">
          ...
          {isLoadingSeats ? (
            ...
            <div className="text-center p-6 text-muted-foreground text-sm font-semibold">Không có dữ liệu sơ đồ ghế.</div>
          ) : (
            <div className="flex flex-col items-center gap-4 py-4">
              <div className="w-4/5 h-1 bg-primary/60 rounded-full shadow-lg shadow-primary/20" />
              <p className="text-[10px] text-muted-foreground font-extrabold tracking-widest -mt-2">MÀN HÌNH CHIẾU</p>
              ...
              <div className="space-y-1.5 mt-4 overflow-x-auto w-full max-w-full pb-2">
                {formattedSeatsGrid.map(row => (
                  <div key={row.rowLabel} className="flex items-center gap-1.5 justify-center min-w-max">
                    <span className="w-5 text-[9px] text-muted-foreground text-center font-bold">{row.rowLabel}</span>
                    <div className="flex gap-1">
                      {row.seats.map((seat, indexInRow) => {
                        ...
                        let seatBg = 'bg-[#27272a]/30 border-border text-muted-foreground';
                        let textColor = 'text-muted-foreground';
                        if (seat.status === 'Booked') {
                          seatBg = 'bg-red-500 border-red-500 text-foreground font-extrabold';
                        } else if (seat.status === 'Held') {
                          seatBg = 'bg-orange-500 border-orange-500 text-foreground font-extrabold';
                        } else {
                          if (seat.seatType === 'VIP') seatBg = 'bg-transparent border border-primary text-primary';
                          else if (seat.seatType === 'Couple') seatBg = 'bg-red-500/10 dark:bg-red-950/20 border border-red-500 dark:border-red-800 text-red-500';
                        }
              ...
              {/* Seat Legend */}
              <div className="flex items-center justify-center flex-wrap gap-4 text-[10px] text-muted-foreground pt-4 border-t border-border w-full mt-4">
                <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-[#27272a]/30 border border-border" /> Thường</div>
                <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-transparent border border-primary" /> VIP</div>
                <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-red-500/10 dark:bg-red-950/20 border border-red-500 dark:border-red-800" /> Ghế Đôi</div>
          ...
          <DialogFooter className="border-t border-border pt-3 flex flex-col sm:flex-row justify-between items-center w-full gap-3">
              ...
              {selectedShowtimeForSeats && (selectedShowtimeForSeats.availableSeats === selectedShowtimeForSeats.totalSeats) && (
                <>
                  <Button
                    ...
                    className="bg-blue-600 hover:bg-blue-700 text-foreground rounded-xl h-10 px-4 text-xs font-bold gap-1.5"
                  >
                    <Pencil className="w-3.5 h-3.5" />
                    Sửa
                  </Button>
                  <Button
                    ...
                    className="bg-red-600 hover:bg-red-700 text-foreground rounded-xl h-10 px-4 text-xs font-bold gap-1.5"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                    Xóa
                  </Button>
            ...
            <Button onClick={() => setIsSeatsDialogOpen(false)} className="bg-muted hover:bg-accent text-foreground rounded-xl h-10 px-6 text-xs w-full sm:w-auto">Đóng</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
